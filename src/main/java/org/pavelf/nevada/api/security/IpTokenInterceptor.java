...
/**
 * If some request invokes service with some token from different IP and 
 * minimal allowed duration between requests with same token and different IP 
 * has not yet been elapsed (e. g. token owner move to different place), token 
 * is discarded. If no token in Access header then just go further in 
 * intercepter chain.  
 * @since 1.0
 * @author Pavel F.
 * */
